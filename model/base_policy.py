...
class ActorCriticPolicy(BasePolicy):
    ...
    def _get_latent(self, obs: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        features = self.extract_features(obs)   ###  features is obs of num_envs environment. (like 10 environment obs)

        latent_pi, latent_vf = self.mlp_extractor(features)
        #print("latent_pi",latent_pi)
        #print("latent_vf",latent_vf)
        #print(" self.mlp_extractor", self.mlp_extractor)
        #assert 1 == 238
        #print("latent_pi",  latent_pi)
        #print("latent_vf", latent_vf)
        #assert 1==239
...
class ContinuousCritic(BaseModel):
    ...
    def __init__(
        ...
        for idx in range(n_critics):
            q_net = create_mlp(features_dim + action_dim + 1, 1, net_arch, activation_fn)
            q_net = nn.Sequential(*q_net)
            self.add_module(f"qf{idx}", q_net)
            self.q_networks.append(q_net)

    def forward(self, obs: th.Tensor, actions: th.Tensor, steps: th.Tensor) -> Tuple[th.Tensor, ...]:
        # Learn the features extractor using the policy loss only
        # when the features_extractor is shared with the actor
        with th.set_grad_enabled(not self.share_features_extractor):
            features = self.extract_features(obs)
        qvalue_input = th.cat([features, actions, steps], dim=1)
        return tuple(q_net(qvalue_input) for q_net in self.q_networks)

    def q1_forward(self, obs: th.Tensor, actions: th.Tensor, steps: th.Tensor) -> th.Tensor:
        """
        ...
        with th.no_grad():
            features = self.extract_features(obs)
        return self.q_networks[0](th.cat([features, actions, steps], dim=1))
...
def create_sde_features_extractor(
    ...
    latent_sde_net = create_mlp(features_dim, -1, sde_net_arch, activation_fn=sde_activation, squash_output=True)
